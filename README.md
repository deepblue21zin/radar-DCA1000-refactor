...
- `Runs`: run 라이브러리, KPI 확인, 세션 HTML 바로 열기, annotation 저장
- `Captures`: raw capture 라이브러리, invalid rate와 linked run 확인
- `Compare`: before / after run KPI 비교
- `Analytics/Triage`: 여러 run을 PMF/ECDF 관점으로 모아 보고, rule-based 병목 원인을 자동 분류
